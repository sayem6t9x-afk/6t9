...
def handle_query(call):
    ...
    if call.data == "action_menu":
        ...
        if not users: return bot.send_message(chat_id, "⚠️ No users found in database.")
        ...
        with open(filename, "rb") as f: bot.send_document(chat_id, f, caption=f"📊 **Total Users:** {len(users)}", parse_mode="Markdown")
        os.remove(filename)
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if user_to_ban == ADMIN_ID:
                return bot.edit_message_text(chat_id=chat_id, message_id=edit_msg_id, text="❌ **Boss, Admin ke ban kora jabe na!**", parse_mode="Markdown", reply_markup=markup)
...
def process_api_key_step(message, edit_msg_id):
    ...
    markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🏠 Main Menu", callback_data="action_menu"))
    try: bot.edit_message_text(chat_id=chat_id, message_id=edit_msg_id, text="⏳ **Verifying API Key...**", parse_mode="Markdown")
    except: pass
...
def handle_document(message):
    ...
    if not message.document.file_name.endswith('.txt'): 
        ...
        return
    try:
        msg = bot.send_message(chat_id, "⏳ **Working... Syncing to Cloud Database**", parse_mode="Markdown")
        track_message(chat_id, msg.message_id)
